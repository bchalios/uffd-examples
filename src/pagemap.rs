...
impl PagemapReader {
    ...
    pub fn new(_page_size: usize) -> Result<Self, PagemapError> {
        ...
        let host_page_size = unsafe { libc::sysconf(libc::_SC_PAGESIZE) } as usize;
    ...
    pub fn read_entry(&self, virt_addr: usize) -> Result<PagemapEntry, PagemapError> {
        let mut entry_bytes = [0u8; 8];
        // Calculate offset for this virtual page (using host page size)
        let host_vpn = virt_addr / self.host_page_size;
        ...
        Ok(PagemapEntry::from_bytes(entry_bytes))
    }
    ...
    pub fn is_page_dirty(&self, virt_addr: usize) -> Result<bool, PagemapError> {
        let entry = self.read_entry(virt_addr)?;
